...
#### 1 видео
90% рисков должны быть управляемые, и только 10% могут быть неснижаемые. Если больше, значит проект рисковый, либо мы ошиблись, и некоторые риски снижаемые.

- **Терминирующий риск** — это риск, который **приводит к невозможности реализации проекта**
- **Неснижаемый риск** — это риск, **который невозможно снизить**. Его должно быть не более 10%

Например, нам дают оборудование. Мы советуемся с экспертами, они говорят, что оно может сломаться, без возможности починить (не хватит бюджета). Мы приходим к заказчику и договариваемся так: 
	 Мы возьмёмся за проект, но у нас есть ==терминирующий, неснижаемый== риск:. если сломается оборудование, то нам не хватит бюджета, и мы не сможем уложиться в рамки. 
	 Мы будем использовать осторожно, но если сломается, то это наша общая проблема, ответственность не на мне. 


#### 2 видео
#### 4 видео Качественный анализ рисков
![Pasted image 20251107150145.png](/img/user/Pasted%20image%2020251107150145.png) 
Мы можем сделать таблицу 5 на 5, добавив "очень Низкое" и "очень Высокое"
А так же, если вероятность очень низкая, то риск всегда будет оцениваться ниже среднего. Например, вероятность того, что метеорит упадёт на офис 

	Риск с низким влиянием и вероятность, не берётся в работу, но мы иногда пересматриваем в ходе проекта, повысилось ли его вероятность и влияние. 
#### 5 видео Количественный анализ рисков
![Pasted image 20251107150841.png|400](/img/user/Pasted%20image%2020251107150841.png)

Например, вероятность 30%, что риск произойдёт. Стоимость его выйдет 20 000 рублей
Вероятность умножаем на влияние и получаем 6000 рублей. столько денег откладываем на резерв.
Если риск случится, то мы берём 20 000 из общих денег, выделенных на риск.
#### 6 видео Пример использования анализа рисков
Если к заказчику постоянно приходить с проблемами, то он на подсознательном уровне будет вас ассоциировать с проблемами. 
Мы можем прийти к заказчику показать какие риски вначале. И прийти попозже, показать, как мы риски проработали, и их количество уменьшилось и их стоимость. Тем самым сэкономив деньги.

#### 7 видео Планирование и реагирование на Риск

План А - Что сделаем, чтобы риск не случился
План Б - Что будем делать, если (КОГДА) риск случится  
Триггер Риска - Симптом наступления Риска
Хозяин Риска - Кто заметит триггер риска и (по возможности) приведёт План Б в исполнение.

Например, есть терминированный риск, что уйдёт Ключевой Сотрудник. 
Мы можем с ним поговорить, выяснить чего он хочет, понять его карьерные планы, поискать точки соприкосновения в этих планах. Договориться о повышении ЗП, при повышении его навыков.

Ранние триггеры:
- Уходит на обед подольше, и внешний вид стал лучше (наверное, ходит на собесы в обед). 
Можно с ним поговорить, спросить, как ему работается, всё-ли у него в порядке.
- Обновил резюме. Обычно, если не ищут работу, то не обновляют резюме.

Хозяин - тот, кто заметит симптом Риска, кому виднее всего. 
В случае с сотрудником - HR. 
Он не только отслеживает риск, но и запускает в работу план Б

Например, новичок может не справляться с работой. Его тимлид может последить за ним. Если он не справляется, то нужно его менять. Тимлид это заметит быстрее. 

Затраты на план А мы точно понесём, потому что будем снижать вероятность Риска.

### Стратегии реагирования на риски
Петя может провалить сроки из-за неопытности.
- Заменить Петю более опытным сотрудником (avoid - уклонение от Риска)
- подучить Петю / дать напарника (mitigate - смягчение Риска). Пытаемся повлиять на вероятность и влияние Риска.
- застраховать риск (transfer - перенос Риска). Переложить ответственность на других (Аутсорсинг). Субподрядчик много заплатит, если не справится. 
- будь что будет (accept)

Мы считаем EMV для позитивных и EMV для негативных рисков. Отдельно.
После этого можем посчитать EMV проекта методом вычитания. Но лучше этого не делать. И спонсору про позитивные риски лучше не говорить.

Весь проект происходит Контроль Рисков, добавление новых, удаление несработавших. Если риск удалили, то резерв риска остаётся. Потому одни риски произойдут, другие нет, и они друг друга компенсируют.

Что делать с непредвиденными Рисками?
Выкручиваться)))  Делать обход.
Неприятные вещи: Сжатие расписания; Урезать сроки; Урезать бюджет; Урезать качество 

==Риски нужно делегировать на Хозяев рисками==

### Папка 19 Управление рисками для PMP

### Папка 20 Управление закупками 
### Папка 22 Управление человеческими ресурсами

#### Тимбилдинг - это построение команды - это ==совместное планирование==, и только потом совместные развлечения.
Тимбилдингом в матричной структуре занимается Project Manager. И он должен больше обладать властью над подчиненными. 
Функциональные руководители (начальники отделов) отвечают за развитие сотрудника, как спеца.
****
Грибной метод - враг тимбилдинга. 
Чтобы выросли хорошие грибы, их нужно покрыть навозом и оставить в темноте. 
Грибной метод: если команде ставить задачку, но не вовлекать в совместное планирование, держать в неведение, то она быстро демотивируется.   
****

Чем больше полномочий, тем больше можем влиять. Если мы не можем влиять на сотрудника (уволить, или удалить из команды), то сложнее управлять.
Есть ещё теория мотивации.

### Матрица RACI (Видео 2)
Есть ещё матрица ГВ (Главный и вспомогательный). если главный заболел, то вспомогательный подстрахует.
- AccountabIe - принимает работу у Responsible. Он всегда есть и всегда один.
- Responsible - исполнитель работы. Может быть много, а может, что только AccountabIe является исполнителем.
- Consult - консультирует, как лучше сделать, но не выполняет.
- Informed - их просто держат в курсе. Например, команда, которая участвует в части проекта, и делает работу после той, которая должна сообщить о готовности своей части работы.


Project Manager не должен быть везде AccountabIe, нужно уметь делегировать, иначе будет не успевать. 

### Теория Мотивации Герцберга (Видео 3)
